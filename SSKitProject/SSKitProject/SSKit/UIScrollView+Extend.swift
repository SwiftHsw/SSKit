...
extension UIScrollView {
    ...
    class func addHeaderRefresh(refreshingBlock: @escaping BlockWithNone) -> MJRefreshNormalHeader {
        ...
        header.stateLabel?.textColor = UIColor.black
    ...
    class func addCustomHeaderRefresh(refreshingBlock: @escaping BlockWithNone) -> SCustomRefreshHeader {
        return SCustomRefreshHeader(refreshingBlock: refreshingBlock)
    }
    
    class func addGifHeaderRefresh(refreshingBlock: @escaping BlockWithNone) -> SCustomGifRefreshHeader {
        let header = SCustomGifRefreshHeader(refreshingBlock: refreshingBlock)
        var images: [UIImage] = []
        for i in 0...64 {
